			...
			if thisArg == 'name':
				...
				for i in range(0, nconditions, 2):
					try:
						...
						raise SMAError('Error creating state: ' + self.stateNames[-1] + '. ' + conditionList[
							i] + ' is an invalid event name.')
				...
				for i in range(0, nactions, 2):
					...
					if thisAction in self.metaOutputNames:
						...
						try:
							...
							raise SMAError('Error creating state: ' + self.stateNames[
								-1] + '. ' + thisAction + ' is an invalid output name.')
						thisOutputValue = outputActionList[i + 1]
					self.outputMatrix[thisStateNumber].append((thisOutputCode, thisOutputValue))
			else:
				raise SMAError(
					'Error: valid state machine arguments are: Name, Timer, StateChangeConditions, OutputActions.')
		self.nStates += 1
